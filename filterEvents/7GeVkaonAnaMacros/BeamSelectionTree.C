#define BeamSelectionTree_cxx
#include "BeamSelectionTree.h"
#include <TH2.h>
#include <TF1.h>
#include <TStyle.h>
#include <TCanvas.h>
...
void BeamSelectionTree::Loop(int PDG)
{
//gSystem->Load("RooUnfold/libRooUnfold");
//   In a ROOT session, you can do:
//      root> .L BeamSelectionTree.C
//      root> BeamSelectionTree t
...
TFile *fout = new TFile(Form("7GeV%sTreeSel.root",fileString.c_str()),"RECREATE");
TTree tree("ana","anaTree");
    int selection_ID=0; int sample_ID=0;
int selection_ID_old=0;
...
int true_beam_daughter_onlyKPlus=0;
...
//tree.Branch("true_beam_daughter_onlyKplus",&true_beam_daughter_onlyKplus);
...
tree.Branch("g4rw_primary_grid_weights",&g4rw_primary_grid_weights);   
...
Long64_t nbytes = 0, nb = 0;
   ...
   reco_daughter_PFP_true_nDaughters=reco_daughter_PFP_true_byHits_ID->size();
     ...
     //for(int daughter=0; daughter=true_beam_daughter_PDG->size(); daughter++){
      // double true_beam_startP=true_beam_startPx->at(daughter)    
...
inelProcess="protonInelastic";
  ...
  // if(reco_beam_len<-800 || reco_beam_calo_wire->size()<1) continue;
   ...
   else if (abs(reco_beam_dCos)<0.9870 || reco_beam_dCos>0.9976 || abs(beam_inst_Y-reco_beam_startY+0.3332)>0.7681 || abs(beam_inst_Z-reco_beam_startZ+29.6274)>1.2639  || abs(beam_inst_X-reco_beam_startX-0.04672)>0.6389 || abs(reco_beam_diffXY-0.3969)>0.6159) selection_ID_old=3;
