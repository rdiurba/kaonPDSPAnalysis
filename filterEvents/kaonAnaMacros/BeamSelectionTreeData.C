#define BeamSelectionTreeData_cxx
#include "BeamSelectionTreeData.h"
#include <TH2.h>
#include <TF1.h>
#include <TStyle.h>
#include <TCanvas.h>
...
void BeamSelectionTreeData::Loop(int PDG)
{
//gSystem->Load("RooUnfold/libRooUnfold");
//   In a ROOT session, you can do:
//      root> .L BeamSelectionTreeData.C
//      root> BeamSelectionTreeData t
...
TFile *fout = new TFile(Form("%sTreeSelData.root",fileString.c_str()),"RECREATE");
...
tree.Branch("reco_beam_calo_endZ",&reco_beam_calo_endZ,"reco_beam_calo_endZ/D");
tree.Branch("reco_beam_calo_startZ",&reco_beam_calo_startZ,"reco_beam_calo_startZ/D");
...
tree.Branch("reco_beam_calibrated_incidentEnergies",&reco_beam_calibrated_incidentEnergies);
   ...
   for (Long64_t jentry=0; jentry<nentries;jentry++) {
    ...
    double dY=reco_beam_endY-reco_beam_startY;
     beam_inst_XY=TMath::Sqrt(TMath::Power(beam_inst_X+30.04,2)+TMath::Power(beam_inst_Y-422.97,2));
...
beam_inst_KE=1000.f*(TMath::Sqrt(beam_inst_P*beam_inst_P+0.9383*0.9383)-0.9383);
   ...
   else if (!(abs(beam_inst_Y-reco_beam_startY+1.8326)<3.2818  && abs(beam_inst_Z-reco_beam_startZ+30.5198)<3.2612  && abs(beam_inst_X-reco_beam_startX+3.9788)<0.8848 && reco_beam_dCos>0.9595 && abs(reco_beam_diffXY-4.4336)<1.4218)) selection_ID=3;
...
if (reco_beam_dEdX_SCE->at(index)>20 || reco_beam_dEdX_SCE->at(index)<0){
interactingKE=interactingKE-currentdEdx*reco_beam_TrkPitch_SCE->at(index);
if(index<reco_beam_dEdX_SCE->size()-1) reco_beam_calibrated_incidentEnergies->push_back(interactingKE);
}
else{
