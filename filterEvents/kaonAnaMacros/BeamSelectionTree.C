...
TFile *fout = new TFile(Form("%sTreeSel.root",fileString.c_str()),"RECREATE");
...
double reco_beam_skippedHits=0;
...
tree.Branch("reco_beam_skippedHits", &reco_beam_skippedHits, "reco_beam_skippedHIts/D");
...
inelProcess="protonInelastic";
  ...
  // if(reco_beam_len<-800 || reco_beam_calo_wire->size()<1) continue;
   else if (abs(reco_beam_dCos)<0.99 || reco_beam_dCos>0.997 || abs(beam_inst_Y-reco_beam_startY-0.36)>0.42 || abs(beam_inst_Z-reco_beam_startZ+29.5)>1.71  || abs(beam_inst_X-reco_beam_startX+2.45)>0.3 || abs(reco_beam_diffXY-2.485)>0.295) selection_ID=3;
   ...
   else if(true_beam_endZ>220.0) sample_ID=2;
...
reco_beam_skippedHits=reco_beam_skippedHits+1;
