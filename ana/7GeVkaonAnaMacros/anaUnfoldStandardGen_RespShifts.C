#define anaUnfoldStandardGen_RespShifts_cxx
#include "anaUnfoldStandardGen_RespShifts.h"
#include <TH2.h>
#include <TStyle.h>
#include <TCanvas.h>
...
void anaUnfoldStandardGen_RespShifts::Loop(std::string weightDecision)
{
//   In a ROOT session, you can do:
//      root> .L anaUnfoldStandardGen_RespShifts.C
//      root> anaUnfoldStandardGen_RespShifts t
...
TFile *fWrite=new TFile(Form("7GeVkaonRespShiftOutputStandardGen_%s.root",weightDecision.c_str()),"recreate");
...
std::string fileName="7GeVkaonUnfoldStandardGenRecoPlots.root";
TFile *fCV=new TFile(Form("%s",fileName.c_str()),"r");
...
TH1D* missesCV=(TH1D*)fCV->Get("h1d_miss_interactingE");
  ...
  TGraph* fSystBeamShiftWidths = (TGraph*)shift_file.Get("gWidths");
   ...
   auto time=ts->AsDouble();
    std::cout<<time<<std::endl;
gRandom->SetSeed(int(time-time+10));
