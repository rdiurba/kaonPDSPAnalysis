...
TFile *fWrite=new TFile(Form("7GeVkaonTrainingRespShiftOutputStandardGen_%s_1000.root",weightDecision.c_str()),"recreate");
...
TFile *fCV=new TFile(Form("7GeVkaonUnfoldStandardGenRecoPlots.root"),"r");
...
gRandom->SetSeed(int(time-time+20));
...
if (reco_beam_calo_wire_z->at(index)>220.0 || reco_beam_calo_wire_z->at(index)<51.8856) continue;
