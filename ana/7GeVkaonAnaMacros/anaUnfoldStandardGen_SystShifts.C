...
TH1D* lastDataHist=new TH1D("lastDataHist", "lastDataHist", totBins, edges);
...
for(int universe=0; universe<nUniverses; universe++){
  ...
  // dEdxshift=1.0; beamshift=1.0;
   ...
   ediv_var=gRandom->Gaus(0.3663,0.20);
   //if (ediv_var>2) ediv_var=1.99;
   ...
   //long_var=gRandom->Gaus(1,0.2);
   long_var=gRandom->Gaus(1,0.03);
   //if (long_var>2) long_var=1.99;
   if (long_var<0) long_var=0.00;
    short_var=gRandom->Gaus(1,0.2);
   //if (short_var>2) short_var=1.99;
   if (short_var<0) short_var=0.00;
   ...
   if (universe>0){   
   double currentCV=(lastDataHist->GetBinContent(1)+lastDataHist->GetBinContent(2))/2;
   double newCVForThrow=currentCV/450;
   double uncertDiff=abs(lastCV-currentCV)/lastCV;
   myfile<<universe<<","<<450*(universe*0.01+0.49)<<","<<currentCV<<std::endl;


    lastCV=currentCV;
   g4rw_index=gRandom->Gaus((universe*0.01+0.5),0.0000001);
}
else myfile<<universe<<","<<450*0.5<<","<<"0"<<std::endl;
...
std::cout<<response.UseOverflowStatus()<<std::endl;
  ...
  if (ediv_weight<0) ediv_weight=0;
...
else matched_weight=(1-0.04338*short_var)/(1-0.04338);
// 0.04338 was 0.374
}
else{
if (reco_beam_true_byE_ID==true_beam_ID) matched_weight=long_var;
else matched_weight=(1-0.82553*long_var)/(1-0.82553);
// 0.8255 was 0.783

}
}

if (matched_weight<0) matched_weight=0;
if (nobeam_weight<0) nobeam_weight=0;
...
else beamScraper_weight=(1-0.06143*beamScraper_var)/(1-0.06143);
...
if (g4rw_weight<0.0) g4rw_weight=0.0;
...
else extTrk_weight=(1-extTrk_var*0.14765)/(1-0.14765);
if (reco_beam_calo_endZ-true_beam_endZ<-10) brkTrk_weight=brkTrk_var;
else brkTrk_weight=(1-brkTrk_var*0.02864)/(1-0.02864);
...
if (reco_beam_calo_wire_z->at(index)<51.8865+startShift || reco_beam_calo_wire_z->at(index)>220.0+endShift) continue;
...
if ((index>=(true_beam_traj_slice_index->size()-1) || true_beam_PDG!=321 || true_beam_ID!=reco_beam_true_byE_ID) && reco_beam_calo_wire_z->at(index)<220.0) responseIncident.Fake(interactingKERW, tot_weight);
  ...
  if (selection_ID==1 && reco_beam_endZ>51.8865+startShift && reco_beam_endZ<220.00+endShift){ 
  ...
  if (true_beam_endZ>30 && true_beam_endZ<222.1056 && true_beam_endProcess->find("kaon+Inelastic")!=std::string::npos) response.Miss(true_beam_traj_interactingEnergy, tot_weight);
...
std::vector<double> tmpVecMC;
      for (int i = 1; i <= xsec_unfoldData->GetNbinsX(); ++i) {
        lastDataHist->SetBinContent(i,xsec_unfoldData->GetBinContent(i));
