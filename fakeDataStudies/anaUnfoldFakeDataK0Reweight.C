#define anaUnfoldFakeDataK0Reweight_cxx
#include "anaUnfoldFakeDataK0Reweight.h"
#include <TH2.h>
#include <TStyle.h>
#include <TCanvas.h>
...
void anaUnfoldFakeDataK0Reweight::Loop(double coeff, int limit)
{
//   In a ROOT session, you can do:
//      root> .L anaUnfoldFakeDataK0Reweight.C
//      root> anaUnfoldFakeDataK0Reweight t
...
TFile *fout = new TFile(Form("kaonUnfoldFakeDataK0Reweight_%1.3f.root",coeff),"RECREATE");
...
int numKaon=0;
      ...
      for(long unsigned int i=0; i<true_beam_daughter_PDG_data->size(); i++){
        ...
        if (true_beam_daughter_PDG_data->at(i)==321) nKaonPlus++;
        if (true_beam_daughter_PDG_data->at(i)==310 || true_beam_daughter_PDG_data->at(i)==130) nKaon0++;
        if (true_beam_daughter_PDG_data->at(i)==-321) nKaonMinus++;
	} 
weight=1.f;
