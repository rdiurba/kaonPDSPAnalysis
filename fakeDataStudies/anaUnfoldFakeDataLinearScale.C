#define anaUnfoldFakeDataLinearScale_cxx
#include "anaUnfoldFakeDataLinearScale.h"
#include <TH2.h>
#include <TStyle.h>
#include <TCanvas.h>
...
void anaUnfoldFakeDataLinearScale::Loop(double coeff)
{
//   In a ROOT session, you can do:
//      root> .L anaUnfoldFakeDataLinearScale.C
//      root> anaUnfoldFakeDataLinearScale t
...
//MC binning 
//double edges[6]={4:700,5000,5200,5400,5620,6020};
//Data binning
...
if(true_beam_endZ>0 && true_beam_endZ<700 && true_beam_endProcess->find("kaon+Inelastic")!=std::string::npos)  h1d_true_thinslice_interactingE->Fill(true_beam_traj_interactingEnergy);
   ...
   for (size_t j = 0; j < true_beam_traj_incidentEnergies->size(); j++) {
        ...
        if (selection_ID>2) responseIncident.Miss(true_beam_traj_incidentEnergies->at(j));
        if(selection_ID<3 && (reco_beam_calibrated_dEdX_SCE->size()-1)<j) responseIncident.Miss(true_beam_traj_incidentEnergies->at(j));
        finalSliceIndex=j;
      }
...
if(true_beam_traj_incidentEnergies->size()){
if (index<(true_beam_slices->size()-1) && true_beam_PDG==321 && true_beam_ID==reco_beam_true_byE_ID && reco_beam_calo_wire_z->at(index)<220.0) responseIncident.Fill(interactingKE,true_beam_traj_incidentEnergies->at(index));

if (index<(true_beam_slices->size()-1) && reco_beam_calo_wire_z->at(index)>220.0) responseIncident.Miss(true_beam_traj_incidentEnergies->at(index));
}
if(!true_beam_traj_incidentEnergies->size()) responseIncident.Fake(interactingKE);
if ((index>=(true_beam_traj_slice_index->size()-1) || true_beam_PDG!=321 || true_beam_ID!=reco_beam_true_byE_ID) && reco_beam_calo_wire_z->at(index)<220.0) responseIncident.Fake(interactingKE);
}
  ...
  if(true_beam_endProcess->find("kaon+Inelastic")!=std::string::npos && reco_beam_true_byE_ID==true_beam_ID /* && reco_beam_daughter_sameID==0*/){ response.Fill(reco_beam_calibrated_interactingEnergy, true_beam_traj_interactingEnergy);
  h1d_reco_cheat_interactingE->Fill(reco_beam_calibrated_interactingEnergy);
  }
  else{
    response.Fake(reco_beam_calibrated_interactingEnergy);
  ...
  if (true_beam_endZ>0 && true_beam_endZ<700 && true_beam_endProcess->find("kaon+Inelastic")!=std::string::npos) response.Miss(true_beam_traj_interactingEnergy);
...
TFile *fout = new TFile(Form("kaonUnfoldStandardGenLinearScale_%0.f.root",g4rw_index),"RECREATE");
...
double weight=1.0;
if (reco_beam_true_byE_ID==true_beam_ID) weight=((true_beam_interactingEnergy-4000)*(g4rw_index/2650))+1; 
...
for(int bin=0; bin<h1d_DataXSec->GetNbinsX(); ++bin){
    for(int bin2=0; bin2<h1d_DataXSec->GetNbinsX(); ++bin2){
      double weight1=((xsec_unfoldData->GetBinContent(bin+1)-4000)*(g4rw_index/2650))+1;
     double weight2=((xsec_unfoldData->GetBinContent(bin2+1)-4000)*(g4rw_index/2650))+1;

    double diffBin1=xsec_unfoldData->GetBinContent(bin+1)-weight1*g4Truth->Eval(xsec_unfoldData->GetBinCenter(bin+1));
