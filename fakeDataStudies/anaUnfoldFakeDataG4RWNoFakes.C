...
if (true_beam_traj_incidentEnergies->size()){
if (index<(true_beam_slices->size()-1) && true_beam_PDG==321 && true_beam_ID==reco_beam_true_byE_ID && reco_beam_calo_Z->at(index)<220.0 && reco_beam_calo_Z->at(index)>30.0) responseIncident.Fill(interactingKE,true_beam_traj_incidentEnergies->at(index));
...
//if ((index>=(true_beam_traj_slice_index->size()-1) || true_beam_PDG!=321 || true_beam_ID!=reco_beam_true_byE_ID) && reco_beam_calo_Z->at(index)>30.0 && reco_beam_calo_Z->at(index)<220.0) responseIncident.Fake(interactingKE,weight);
  ...
  if(true_beam_endZ>30 && true_beam_endZ<220.0 && true_beam_endProcess->find("kaon+Inelastic")!=std::string::npos && reco_beam_true_byE_ID==true_beam_ID /* && reco_beam_daughter_sameID==0*/){ response.Fill(reco_beam_calibrated_interactingEnergy, true_beam_traj_interactingEnergy);
 ...
 //   response.Fake(reco_beam_calibrated_interactingEnergy);
  ...
  if (true_beam_endZ>30 && true_beam_endZ<220.0 && true_beam_endProcess->find("kaon+Inelastic")!=std::string::npos) response.Miss(true_beam_traj_interactingEnergy);
