#define anaUnfoldFakeDataG4RWNoFakes_cxx
#include "anaUnfoldFakeDataG4RWNoFakes.h"
#include <TH2.h>
#include <TStyle.h>
#include <TCanvas.h>
...
void anaUnfoldFakeDataG4RWNoFakes::Loop(double coeff)
{
//   In a ROOT session, you can do:
//      root> .L anaUnfoldFakeDataG4RWNoFakes.C
//      root> anaUnfoldFakeDataG4RWNoFakes t
...
if (true_beam_traj_incidentEnergies->size()){
if (index<(true_beam_slices->size()-1) && true_beam_PDG==321 && true_beam_ID==reco_beam_true_byE_ID && reco_beam_calo_wire_z->at(index)<220.0 && reco_beam_calo_wire_z->at(index)>51.8865) responseIncident.Fill(interactingKE,true_beam_traj_incidentEnergies->at(index));
...
//if ((index>=(true_beam_traj_slice_index->size()-1) || true_beam_PDG!=321 || true_beam_ID!=reco_beam_true_byE_ID) && reco_beam_calo_wire_z->at(index)>51.8865 && reco_beam_calo_wire_z->at(index)<220.0) responseIncident.Fake(interactingKE,weight);
  ...
  if(true_beam_endZ>30 && true_beam_endZ<222.1056 && true_beam_endProcess->find("kaon+Inelastic")!=std::string::npos && reco_beam_true_byE_ID==true_beam_ID /* && reco_beam_daughter_sameID==0*/){ response.Fill(reco_beam_calibrated_interactingEnergy, true_beam_traj_interactingEnergy);
 ...
 //   response.Fake(reco_beam_calibrated_interactingEnergy);
  ...
  if (true_beam_endZ>30 && true_beam_endZ<222.1056 && true_beam_endProcess->find("kaon+Inelastic")!=std::string::npos) response.Miss(true_beam_traj_interactingEnergy);
...
TFile *fout = new TFile(Form("kaonUnfoldDatadrivenG4RWNoFakes_%d.root",g4rw_index+1),"RECREATE");
...
else if(reco_beam_true_byE_ID!=true_beam_ID)  continue;
