...
TFile f16(Form("kaonUnfoldFakeDataScaleForward_0.000.root"));
TFile f6(Form("kaonUnfoldFakeDataScaleForward_2.000.root"));
...
//g414->Draw("SAME");
  ...
  lErr->AddEntry(recoXSec6,Form("Sec. Kaon with cos(#theta)>0.995 for all 1 K+X tracks evts. reweight by 2: #chi^{2}=%s",recoXSec6->GetTitle()),"l");
  lErr->AddEntry(recoXSec16,Form("Sec. Kaon with cos(#theta)>0.995 for all 1 K+X tracks evts. reweight by 0: #chi^{2}=%s",recoXSec16->GetTitle()),"l");
...
lErr->Draw("SAME");
c1.Print(Form("xSecUnfoldedFakeDataStandardGenScaleAngle.png"));
