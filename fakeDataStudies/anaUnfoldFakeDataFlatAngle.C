#define anaUnfoldFakeDataFlatAngle_cxx
#include "anaUnfoldFakeDataFlatAngle.h"
#include <TH2.h>
#include <TStyle.h>
#include <TCanvas.h>
...
void anaUnfoldFakeDataFlatAngle::Loop(double coeff,int threshPart)
{
//   In a ROOT session, you can do:
//      root> .L anaUnfoldFakeDataFlatAngle.C
//      root> anaUnfoldFakeDataFlatAngle t
...
TFile *fout = new TFile(Form("kaonUnfoldFakeDataFlatAngle_%1.3f_%d.root",coeff,threshPart),"RECREATE");
...
double nBinsAngle=20;
Double_t edgesAngle[21];
edges[0]=-1;
for (int i=0; i<21; i++) edgesAngle[i]=coeff+((1-coeff)/20.0)*(i);

TH1D* histLeadingAngle=new TH1D("histLeadingAngle","histLeadingAngle",nBinsAngle,edgesAngle);
TH1D* weightLeadingAngle=new TH1D("weightLeadingAngle","weightLeadingAngle",nBinsAngle,edgesAngle);
TH1D* reweightLeadingAngle=new TH1D("reweightLeadingAngle","reweightLeadingAngle",nBinsAngle,edgesAngle);
...
TH1D* h1d_reco_incidentE_data=new TH1D("h1d_reco_incidentE_data","h1d_reco_incidentE_data",totBins, edges);
   ...
   t->GetEntry(jentry);
        ...
        for(long unsigned int i=0; i<true_beam_daughter_PDG_data->size(); i++){
           if (abs(true_beam_daughter_PDG_data->at(i))!=321) continue;
        ...
        if (leadingAngle>-100 && numKaon<=threshPart) histLeadingAngle->Fill(leadingAngle);
}
}
...
if ( numPart<=threshPart && leadingAngle>coeff && true_beam_endZ_data>30 && true_beam_endZ_data<222.1056 && true_beam_endProcess_data->find("kaon+Inelastic")!=std::string::npos) weight=weightLeadingAngle->Interpolate(leadingAngle);

//if (numPart==1 && leadingAngle>0.99 && true_beam_endZ_data>30 && true_beam_endZ_data<222.1056 && true_beam_endProcess->find("kaon+Inelastic")!=std::string::npos) weight=2;
//else if(numPart==1 && true_beam_endZ_data>30 && true_beam_endZ_data<222.1056 && true_beam_endProcess->find("kaon+Inelastic")!=std::string::npos) weight=(1-2*histLeadingAngle->GetBinContent(20)/histLeadingAngle->GetEntries())/(1-histLeadingAngle->GetBinContent(20)/histLeadingAngle->GetEntries());
