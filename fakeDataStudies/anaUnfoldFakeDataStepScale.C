#define anaUnfoldFakeDataStepScale_cxx
#include "anaUnfoldFakeDataStepScale.h"
#include <TH2.h>
#include <TStyle.h>
#include <TCanvas.h>
...
void anaUnfoldFakeDataStepScale::Loop(double coeff)
{
//   In a ROOT session, you can do:
//      root> .L anaUnfoldFakeDataStepScale.C
//      root> anaUnfoldFakeDataStepScale t
...
TFile *fout = new TFile(Form("kaonUnfoldStandardGenStepScale_%1.1f.root",g4rw_index),"RECREATE");
...
double weight=1.0;
if (reco_beam_true_byE_ID==true_beam_ID && true_beam_interactingEnergy>5340) weight=g4rw_index; 
...
for(int bin=0; bin<h1d_DataXSec->GetNbinsX(); ++bin){
    ...
    double weight=1.f;
    double weight2=1.f;
    if (h1d_DataXSec->GetBinCenter(bin+1)>5400) weight=coeff;
    if (h1d_DataXSec->GetBinCenter(bin2+1)>5400) weight2=coeff;
