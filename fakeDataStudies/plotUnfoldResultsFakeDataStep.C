...
TFile f6(Form("kaonUnfoldStandardGenStepScale_%1.1f.root",coeff));
...
if (g46->GetX()[i]>5400) g46->GetY()[i] *= coeff ;
...
//g412->Draw("SAME");
//g414->Draw("SAME");
  TLegend *lErr = new TLegend(0.2,0.7,0.7,0.9);
  lErr->AddEntry(recoXSec6,Form("Fake Data: Weight True Int. Points by %1.2f if KE>5.4 GeV",coeff),"lp");
...
lErr->Draw("SAME");
c1.Print(Form("xSecUnfoldedFakeDataStepScaleStandardGen_%1.2f.png",coeff));
}
