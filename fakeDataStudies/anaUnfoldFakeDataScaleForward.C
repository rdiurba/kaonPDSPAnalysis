...
TH1D* h1d_reco_incidentE_data=new TH1D("h1d_reco_incidentE_data","h1d_reco_incidentE_data",totBins, edges);
   ...
   t->GetEntry(jentry);
        ...
        if (true_beam_endZ_data>30 && true_beam_endZ_data<220.0 && true_beam_endProcess_data->find("kaon+Inelastic")!=std::string::npos) {
        int numPart=0;
        int numKaon=0;
...
//if ( true_beam_endZ_data>30 && true_beam_endZ_data<220.0 && true_beam_endProcess->find("kaon+Inelastic")!=std::string::npos) weight=weightLeadingAngle->Interpolate(leadingAngle);

if ( numKaon==1 && leadingAngle>0.995 && true_beam_endZ_data>30 && true_beam_endZ_data<220.0 && true_beam_endProcess_data->find("kaon+Inelastic")!=std::string::npos) weight=coeff;
else if( numKaon==1 && true_beam_endZ_data>30 && true_beam_endZ_data<220.0 && true_beam_endProcess_data->find("kaon+Inelastic")!=std::string::npos) weight=(1-coeff*histLeadingAngle->GetBinContent(20)/histLeadingAngle->GetEntries())/(1-histLeadingAngle->GetBinContent(20)/histLeadingAngle->GetEntries());
