#define anaUnfoldFakeDataScaleForward_cxx
#include "anaUnfoldFakeDataScaleForward.h"
#include <TH2.h>
#include <TStyle.h>
#include <TCanvas.h>
...
void anaUnfoldFakeDataScaleForward::Loop(double coeff, int limit)
{
//   In a ROOT session, you can do:
//      root> .L anaUnfoldFakeDataScaleForward.C
//      root> anaUnfoldFakeDataScaleForward t
...
TFile *fout = new TFile(Form("kaonUnfoldFakeDataScaleForward_%1.3f_maxTracks%d.root",coeff,limit),"RECREATE");
...
TH1D* histLeadingAngle=new TH1D("histLeadingAngle","histLeadingAngle",20,0.95,1);
TH1D* weightLeadingAngle=new TH1D("weightLeadingAngle","weightLeadingAngle",20,0.95,1);
TH1D* reweightLeadingAngle=new TH1D("reweightLeadingAngle","reweightLeadingAngle",20,0.95,1);
...
TH1D* h1d_reco_incidentE_data=new TH1D("h1d_reco_incidentE_data","h1d_reco_incidentE_data",totBins, edges);
   ...
   t->GetEntry(jentry);
         double leadingAngle=-100;
        if (true_beam_endZ_data>30 && true_beam_endZ_data<222.1056 && true_beam_endProcess_data->find("kaon+Inelastic")!=std::string::npos) {
        int numPart=0;
        int numKaon=0;
        for(long unsigned int i=0; i<true_beam_daughter_PDG_data->size(); i++){
           if (abs(true_beam_daughter_PDG_data->at(i))!=321 && abs(true_beam_daughter_PDG_data->at(i))!=2212 && abs(true_beam_daughter_PDG_data->at(i))!=13 && abs(true_beam_daughter_PDG_data->at(i))!=211) continue;
        if (true_beam_daughter_len_data->at(i)<1.0) continue; 
        double totalStartP=true_beam_daughter_startP_data->at(i);
         
            double daughterDirCos=true_beam_endPx_data/true_beam_endP_data*true_beam_daughter_startPx_data->at(i)/totalStartP+true_beam_endPy_data/true_beam_endP_data*true_beam_daughter_startPy_data->at(i)/totalStartP+true_beam_endPz_data/true_beam_endP_data*true_beam_daughter_startPz_data->at(i)/totalStartP;
        if (abs(true_beam_daughter_PDG_data->at(i))==321) numKaon++;
        numPart++;
        if (daughterDirCos<leadingAngle)  continue;
         leadingAngle=daughterDirCos;
          }  

        if (leadingAngle>-100 && numKaon==1 && numPart==limit) histLeadingAngle->Fill(leadingAngle);
}
}
    double totalLeadingAngles=histLeadingAngle->Integral();
    double averageValueBin=totalLeadingAngles/(histLeadingAngle->GetNbinsX());

     for (int i = 1; i <= weightLeadingAngle->GetNbinsX(); ++i) {
     weightLeadingAngle->SetBinContent(i,averageValueBin/histLeadingAngle->GetBinContent(i));
     reweightLeadingAngle->SetBinContent(i,weightLeadingAngle->GetBinContent(i)*histLeadingAngle->GetBinContent(i));
...
double weight=1.0;
//if (reco_beam_true_byE_ID==true_beam_ID && true_beam_endZ_data<700 && true_beam_endZ_data>0) weight=tot_g4rw[g4rw_index]; 
double leadingAngle=-100;
int numPart=0;
int numKaon=0;
        if (true_beam_endZ_data>30 && true_beam_endZ_data<222.1056 && true_beam_endProcess_data->find("kaon+Inelastic")!=std::string::npos) {        
...
//if ( true_beam_endZ_data>30 && true_beam_endZ_data<222.1056 && true_beam_endProcess->find("kaon+Inelastic")!=std::string::npos) weight=weightLeadingAngle->Interpolate(leadingAngle);

if (numPart==limit && numKaon==1 && leadingAngle>0.995 && true_beam_endZ_data>30 && true_beam_endZ_data<222.1056 && true_beam_endProcess_data->find("kaon+Inelastic")!=std::string::npos) weight=coeff;
else if(numPart==limit && numKaon==1 && true_beam_endZ_data>30 && true_beam_endZ_data<222.1056 && true_beam_endProcess_data->find("kaon+Inelastic")!=std::string::npos) weight=(1-coeff*histLeadingAngle->GetBinContent(20)/histLeadingAngle->GetEntries())/(1-histLeadingAngle->GetBinContent(20)/histLeadingAngle->GetEntries());


if (weight<0) weight=0.0000;

//weightLeadingAngle->Interpolate(leadingAngle);
...
if (reco_beam_calo_wire_z_data->at(index)<220.0 && reco_beam_calo_wire_z_data->at(index)>51.8865) h1d_reco_incidentE_data->Fill(interactingKE,weight);
}
...
h1d_MCInt->Write();
h1d_DataInt->Write();
histLeadingAngle->Write();
weightLeadingAngle->Write();
reweightLeadingAngle->Write();
