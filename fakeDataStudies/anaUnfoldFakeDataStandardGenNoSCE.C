#define anaUnfoldFakeDataStandardGenNoSCE_cxx
#include "anaUnfoldFakeDataStandardGenNoSCE.h"
#include <TH2.h>
#include <TStyle.h>
#include <TCanvas.h>
...
void anaUnfoldFakeDataStandardGenNoSCE::Loop()
{
//   In a ROOT session, you can do:
//      root> .L anaUnfoldFakeDataStandardGenNoSCE.C
//      root> anaUnfoldFakeDataStandardGenNoSCE t
...
std::cout<<response.UseOverflowStatus()<<std::endl;
  ...
  double nobeam_var=0.9772;
  double bqt_var=1.290625;
  double nobeam_weight=nobeam_var;
  if (selection_ID<4){

  //nobeam_weight=1+(relative_nobeam_var)*nobeam_var;
  nobeam_weight=(1-nobeam_var*0.44)/(1-0.44);
    
  }
  if (selection_ID==3){

  nobeam_weight=(1-bqt_var*0.32)/(1-0.32)*nobeam_weight;
  }
  if (selection_ID<3){
  double if_nobeam=(1-nobeam_var*0.44)/(1-0.44);
  double if_failedbqt=bqt_var;//(1-bqt_var*0.23)/(1-0.23);
  nobeam_weight=if_nobeam*if_failedbqt;//(1-if_nobeam*(1-0.44)-if_failedbqt*(1-0.23))/(1-(1-0.44)-(1-0.23));


  }
double sce_weight=1.f;
if (selection_ID<3){
if (true_beam_endZ>0 && true_beam_endZ<50) sce_weight=1.22;
else sce_weight=(1-0.27*1.22)/(1-0.27);
...
double tot_weight=nobeam_weight*sce_weight;
   
//if (selection_ID==4 || selection_ID==3) continue;
if(true_beam_endZ>0 && true_beam_endZ<220 && true_beam_endProcess->find("kaon+Inelastic")!=std::string::npos)  h1d_true_thinslice_interactingE->Fill(true_beam_traj_interactingEnergy);
   ...
   for (size_t j = 0; j < true_beam_traj_incidentEnergies->size(); j++) {
        ...
        //if (true_beam_traj_slice_z->at(j)>220.0 )  continue;
...
if(true_beam_traj_incidentEnergies->size()){
if (index<(true_beam_slices->size()-1) && true_beam_PDG==321 && true_beam_ID==reco_beam_true_byE_ID && reco_beam_calo_wire_z->at(index)<220.0) responseIncident.Fill(interactingKE,true_beam_traj_incidentEnergies->at(index),tot_weight);

//if (index<(true_beam_slices->size()-1) && reco_beam_calo_wire_z->at(index)>220.0) responseIncident.Miss(true_beam_traj_incidentEnergies->at(index),tot_weight);
}
if(!true_beam_traj_incidentEnergies->size()) responseIncident.Fake(interactingKE,tot_weight);
if ((index>=(true_beam_traj_slice_index->size()-1) || true_beam_PDG!=321 || true_beam_ID!=reco_beam_true_byE_ID) && reco_beam_calo_wire_z->at(index)<220.0) responseIncident.Fake(interactingKE,tot_weight);
}
  ...
  if(true_beam_endProcess->find("kaon+Inelastic")!=std::string::npos && reco_beam_true_byE_ID==true_beam_ID /* && reco_beam_daughter_sameID==0*/){ response.Fill(reco_beam_calibrated_interactingEnergy, true_beam_traj_interactingEnergy,tot_weight);
  h1d_reco_cheat_interactingE->Fill(reco_beam_calibrated_interactingEnergy);
  }
  else{
    response.Fake(reco_beam_calibrated_interactingEnergy,tot_weight);
  ...
  if (true_beam_endZ>0 && true_beam_endZ<220 && true_beam_endProcess->find("kaon+Inelastic")!=std::string::npos) response.Miss(true_beam_traj_interactingEnergy,tot_weight);
...
TFile *fout = new TFile(Form("kaonUnfoldStandardGenFakeDataNoSCE.root"),"RECREATE");
...
TFile f("/dune/data/users/rdiurba/offStandard6GeVkaonTreeSel.root");
